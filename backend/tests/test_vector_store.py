...
def test_course_name_resolution(mock_chroma_collection):
    ...
    store = VectorStore(
        chroma_path=":memory:", embedding_model="all-MiniLM-L6-v2", max_results=5
    )
    store.course_catalog = mock_chroma_collection
    ...
    assert (
        resolved == "Test Course: Introduction to Testing"
...
def test_search_error_handling(mock_chroma_collection):
    ...
    assert (
        "ChromaDB connection error" in results.error
...
def test_search_with_course_filter(mock_chroma_collection):
    ...
    results = store.search(query="testing basics", course_name="Test Course")
...
def test_search_with_lesson_filter(mock_chroma_collection):
    ...
    results = store.search(query="testing basics", lesson_number=1)
...
def test_search_results_is_empty():
    ...
    non_empty = SearchResults(
        documents=["doc"], metadata=[{"key": "value"}], distances=[0.5]
    )
